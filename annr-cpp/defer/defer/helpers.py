...
def _callback_loop(defer, density, num_fn_calls, callback, callback_freq_fn_calls, savepoint_callback=None):
  savepoints = np.logspace(1, 6, 31).astype(int)[2:]
  cur_sp_i = 0
  first_call = True
  ...
  for i, _ in enumerate(defer(num_fn_calls)):
    ...
    if cur_sp_i < len(savepoints) and density.num_partitions >= savepoints[cur_sp_i]:
      if savepoint_callback is not None:
        savepoint_callback(density, savepoints[cur_sp_i])
      cur_sp_i += 1
